    ...
    <form method="POST" action="{{ route('tasks.store') }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" id="title" required
                   class="mt-1 w-full border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 @error('title') border-red-500 @enderror" value="{{ old('title') }}">
            @error('title')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description"
                      class="mt-1 w-full border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500">{{ old('description') }}</textarea>
            @error('description')
                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="flex justify-end">
            <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Create Task</button>
        </div>
    </form>
</div>
@endsection